...
const techIcons = {
  'React':'\uead3','Next.js':'\ueb8e','Python':'\ue86f','Flask':'\uf1b7','Supabase':'\uf20e','Vercel':'\uf15c','HTML5':'\ueb7e','CSS3':'\ueb93','JavaScript':'\ueb7c','TypeScript':'\uf84d','Tailwind':'\ue40a','Redux':'\ue97a','MongoDB':'\ue1db','Express':'\ue328','Node.js':'\ueb7c','TensorFlow':'\uea4a','PyTorch':'\ue10e','Scikit-learn':'\uef3e','LangChain':'\ue250','OpenAI':'\uf06c','Claude API':'\uf5d0','Hugging Face':'\uf008','Git':'\ueaf5','Docker':'dock','AWS':'cloud_queue'
}
            ...
            <div className="flex w-max animate-marquee hover:[animation-play-state:paused] opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
              {[...techStack, ...techStack].map((tech, i) => (
                <div key={i} className="flex items-center gap-2 font-code-display text-on-surface whitespace-nowrap px-6">
                  <span className="material-symbols-outlined text-primary">{techIcons[tech] || '\ue86f'}</span>
                  {tech}
                </div>
              ))}
            </div>
    ...
    "status": "Graduated"
        ...
        <section className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[
              { icon: '\ue53b', title: 'Full Stack Development', desc: 'Scalable web applications built with Next.js, React, and robust backend architectures.' },
              { icon: '\uea4a', title: 'AI Systems Architecture', desc: 'Integrating LLMs and machine learning models into production-ready software solutions.' },
              { icon: '\ue8c1', title: 'API Design & Automation', desc: 'Designing RESTful APIs and workflow automations using n8n and custom Python scripts.' },
              { icon: '\uf190', title: 'Data Analysis', desc: 'Transforming complex datasets into actionable insights with interactive visualizations.' },
            ].map(({ icon, title, desc }) => (
